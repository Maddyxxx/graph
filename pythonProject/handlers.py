...
def generate_ticket(text, context):
    ...
    ticket = make_ticket(fio=data['name'], from_=data['dep_city'], to=data['dest_city'], date=data['fly_date'])
    return ticket


def handle_date(text, context):
    manager = Manager(text)
    response = manager.check_date()
    if response is False:
        context['date'] = text
        context['forecast'] = manager.parse_forecast()
        return 'next'
    else:
...
def handle_add_forecast(text, context):
    if text.lower() == 'да':
        context['next_message'] = 'Прогноз добавлен'
        # manager.write_data_to_db(context['forecast'])
        return 'next'
    elif text.lower() == 'нет':
        context['next_message'] = None
        return 'next'
    else:
        context['error'] = 'Введено неверное значение. Введите "да" или "нет"'
        return False


def handle_get_forecast(text, context):
    if text.lower() == 'да':
        try:
            # context['forecast'] = manager.get_data_from_db(context['date'])
            context['next_message'] = context['forecast']
        except Exception:
            context['next_message'] = 'Прогноз из базы не получен, тк вы ничего не добавили в базу'
        return 'next'
    elif text.lower() == 'нет':
        return 'step_4'
    else:
...
def handle_create_postcard(text, context):
    if text.lower() == 'да':
        # manager.make_postcards(context['forecast'])
        return 'next'
    elif text.lower() == 'нет':
        return 'next'
    else:
...
def handle_print_forecast(text, context):
    if text.lower() == 'да':
        context['next_message'] = str()
        for forecast in context['forecast']:
            for item, value in forecast.items():
                context['next_message'] += f'{item}: {value}; '
